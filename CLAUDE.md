...
- `buildSkillxRunCommand(skill, prompt)` emits the full `runUrl` (e.g. `https://github.com/...`) rather than the `github:` shorthand. The shorthand lives on in `skill.commandSource` for the Famous Skills reference table in docs.

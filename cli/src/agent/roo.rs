...
impl AgentAdapter for RooAdapter {
    ...
    async fn detect(&self) -> DetectResult {
                ...
                if ext_dir.is_dir() {
                    if let Ok(entries) = std::fs::read_dir(&ext_dir) {
                        for entry in entries.flatten() {
                            ...
                            if name.starts_with("rooveterinaryinc.roo-cline-") {
        ...
        DetectResult {
            ...
            info: if has_extension {
                Some("VS Code extension rooveterinaryinc.roo-cline found".into())
            } else {
                None
            },
        }
    ...
    fn inject_path(&self, skill_name: &str, scope: &Scope) -> PathBuf {
        match scope {
            Scope::Project => PathBuf::from(".roo").join("skills").join(skill_name),
    ...
    async fn launch(&self, config: LaunchConfig) -> Result<SessionHandle> {
        if let Some(ref prompt) = config.prompt {
            if let Ok(mut clipboard) = arboard::Clipboard::new() {
                ...
                crate::ui::info("Prompt copied to clipboard. Paste it into Roo Code chat.");
            }
        }

        crate::ui::info("Skill injected. Open VS Code with Roo Code and use the skill.");
